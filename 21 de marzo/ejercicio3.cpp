...
//  funcion sumatoria
//tambien declamos  funcion del valor maximo
 int valor_maximo(int matriz_max[5][4], int fila , int columnas){
 for (int i = 0; i < fila; i++) {
      for (int j = 0; j < columnas; j++) {
        

         // Si el valor es mayor que el máximo actual, actualizamos el máximo
         if (matriz_max[i][j] > maximo) {
            maximo = matriz_max[i][j];
         }
      }
   }
